...
@section('title', 'Admin | Edit Data Status')
...
<div class="container-fluid">
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary text-center">Edit Status</h6>
        </div>

        <div class="card-body">
            <form class=" form-signin" action="/status/{{ $Statuses->id }}" method="POST">
                @method('put')
                @csrf
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control @error('name') is-invalid @enderror" id="name"
                        placeholder="Masukan Nama Status" name="name" value="{{ $Statuses->name }}">
                    @error('name')
                ...
                <div class="form-group">
                    <label for="color">Nama Status</label>
                    <input type="text" class="form-control colorpicker @error('color') is-invalid @enderror" id="color"
                        placeholder="Masukan Warna Status" name="color" value="{{ $Statuses->color }}">
                    @error('color')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
